namespace :s3_direct_upload do
  ...
  task :clean_remote_uploads => :environment do
    ...
    s3     = Fog::Storage.new(provider: "AWS", aws_access_key_id: S3DirectUpload.config.access_key_id, aws_secret_access_key: S3DirectUpload.config.secret_access_key, region: S3DirectUpload.config.region)
